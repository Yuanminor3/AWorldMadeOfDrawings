    ...
    update(deltaTime: number): void
    {
        ...
        if (this.drawState == DrawState.NO_DRAWING) {
            ...
            if (this.cameraControls.hasMoved) {
                ...
                // Create a ray that goes directly downwards from the camera's current position
                const downRay = new gfx.Ray3();
                downRay.set(this.camera.position, new gfx.Vector3(0, -1, 0)); // pointing straight down

                // Check for intersection with the ground mesh
                //const vertices3D: gfx.Vector3[] = [];
                // Iterate over each vertex in the stroke2D object
                
                const intersection = downRay.intersectsMesh3(this.ground);
                if (intersection) {
                    // Add the intersection point to the vertices3D array
                    // Set the camera's Y position to the intersection point's Y plus a small offset
                    // so that it is not exactly on the ground, but slightly above it
                    const cameraHeightAboveGround = 2;
                    this.camera.position.y = intersection.y + cameraHeightAboveGround;
                }
