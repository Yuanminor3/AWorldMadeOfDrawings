...
export class Ground extends gfx.Mesh3
{
    ...
    public reshapeGround(stroke2D: Stroke2D, groundStartPoint: gfx.Vector3,  groundEndPoint: gfx.Vector3, camera: gfx.Camera): void
    {
        ...
        const dir = gfx.Vector3.subtract(groundEndPoint, groundStartPoint);
        const up = new gfx.Vector3(0,1,0);
        const normal = gfx.Vector3.cross(dir, up);
        normal.normalize();
        const projectionPlane = new gfx.Plane3(groundStartPoint, normal);

        const projectedStroke: gfx.Vector3[] = [];
        // Iterate over each vertex in the stroke2D object
        for (const vertex of stroke2D.vertices) {
            const ray = new gfx.Ray3();
            //ray.setPickRay(new gfx.Vector2(vertex.x, vertex.y), camera);
            ray.setPickRay(vertex, camera);
        
            const intersection = ray.intersectsPlane(projectionPlane);
            if (intersection) {
                // Add the intersection point to the projectedStroke array
                projectedStroke.push(intersection);
            }
            
        }

        // Loop through all vertices of the ground mesh
        for (const vertex of this.vertices) {
            // Find the closest point on the silhouette curve to this vertex
            const closestPoint = projectionPlane.project(vertex);

            // Calculate the new height value using the computeH function
            const h = this.computeH(closestPoint, projectedStroke, projectionPlane);

            // Calculate the distance from the vertex to the projection plane
            const d = projectionPlane.distanceTo(vertex);

            // Compute the weighting function w(d)
            const w = Math.max(0, 1 - (d / 5) ** 2);

            // Calculate the new y-coordinate for the vertex
            let newY = vertex.y;
            if (h !== 0) {
                newY = ((1 - w) * vertex.y) + (w * h);
            }
            vertex.y = newY; // Update the vertex's y-coordinate with the new height
        }
